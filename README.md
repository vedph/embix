...
- a set of **tokenizers** definitions, each with its own, arbitrary ID. This ID is recalled in other sections of the same profile. Each tokenizer is an object with besides this ID has a `TypeId` (referred to the tokenizer's unique tag) and the ID of a filters chain, when requiring filters.
...
```bash
./embix build-index <JsonFilePath> <DatabaseName> [-t <DatabaseType>] [-c] [-p <PartitionCount>] [-s <MinPartitionSize>] [-l <RecordLimit>]
```
